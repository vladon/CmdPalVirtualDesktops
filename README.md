...
- **Dock band auto-recovers across session switches** — RDP/console transitions still make the palette host drop its band snapshot (tracked upstream in [microsoft/PowerToys#50367](https://github.com/microsoft/PowerToys/issues/50367)), so on those transitions the extension quietly bounces the palette process for you — the band comes back on its own in a few seconds.
