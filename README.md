...
- **Focus lands on the target desktop** — switching from the dock focuses the last-used window on that desktop instead of leaving focus on the dock itself ([original issue #1](https://github.com/zadjii/CmdPalVirtualDesktops/issues/1)).
- **Dock band survives session switches** — RDP/console transitions used to blank the band: the host drops its band binding when item updates arrive mid-transition (tracked upstream in [microsoft/PowerToys#50367](https://github.com/microsoft/PowerToys/issues/50367)). The extension now defers all updates until the session settles and applies them once — the band self-heals with no restarts and no flicker.
