...
- `VirtualDesktopBand.cs` — `IExtension` + `IDisposable`. The `[Guid]` **must** match the `com:Class Id` in `Package.appxmanifest`. The host releases idle extensions after a while; honoring that killed the process and left the dock band dead (the host never re-activates a released extension), so `Dispose()` **deliberately ignores** the release and `Program.Main` runs a watchdog that exits only when the `Microsoft.CmdPal.UI` process itself is gone.
- `VirtualDesktopsListPage` (in `VirtualDesktopCommandsProvider.cs`) is the core: subscribes to `VirtualDesktop.CurrentChanged` / `Created` / `Destroyed` / `Renamed` and `SettingsChanged`, then refreshes via `UpdateDesktopsOffUiThread` → `Task.Factory.StartNew(..., _scheduler)` (captured `TaskScheduler`) → `RaiseItemsChanged()`. `GetItems()` maps each `WindowsDesktop.VirtualDesktop` to a `ListItem`. (`Destroyed` matters for RDP sessions: the session desktop is destroyed on disconnect, and without that subscription the dock band goes stale until it is re-pinned.) **Session-transition protocol**: item updates fired into the host mid-transition break its dock band binding, so `UpdateDesktopsOffUiThread` suppresses refreshes for 8 s around session switches (`SystemEvents.SessionSwitch` sets `_sessionSettleUntil`) and one deferred refresh runs after the settle window; band items are also **mutated in place** (cached in `_itemsByDesktopId`) because the dock renders the ListItem objects it holds at registration and ignores `RaiseItemsChanged`.
